...
class AtomicMultipolesMACE(torch.nn.Module):
    def __init__(
        ...
        correlation: int,
        gate: Callable | None,
        atomic_energies: None = None,  # Just here to make it compatible with energy models, MUST be None
        apply_cutoff: bool = True,  # pylint: disable=unused-argument
        use_reduced_cg: bool = True,  # pylint: disable=unused-argument
        use_so3: bool = False,  # pylint: disable=unused-argument
        distance_transform: str = "None",  # pylint: disable=unused-argument
        ...
        edge_irreps: Optional[o3.Irreps] = None,  # pylint: disable=unused-argument
        dipole_only: Optional[bool] = True,  # pylint: disable=unused-argument
        max_multipole_l: int = 2,  # default to quadrupoles
        ...
        # Predefine buffers to be TorchScript-safe
        n_components = sum(2 * ang + 1 for ang in range(max_multipole_l + 1))

        self.register_buffer('multipole_mean', torch.zeros(n_components))
        self.register_buffer('multipole_std', torch.ones(n_components))
        self.register_buffer('l_start_indices', torch.tensor([x * x for x in range(0, max_multipole_l + 1)]))
        self.register_buffer("change_of_basis", get_change_of_basis())
